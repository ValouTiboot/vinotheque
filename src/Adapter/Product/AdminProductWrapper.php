...
class AdminProductWrapper
{
    ...
    public function processProductAttribute($product, $combinationValues)
    {
        ...
        $product->updateAttribute(
            ...
            $combinationValues['attribute_shop_quantity'],
            $combinationValues['attribute_packaging_price']
        );
        ...
        if(isset($combinationValues['attribute_shop_quantity'])){
            $this->processShopQuantityUpdate($product, $combinationValues['attribute_shop_quantity'], $id_product_attribute);
        }
     ...
     * @param integer $forAttributeId
     */
    public function processShopQuantityUpdate(\ProductCore $product, $shop_quantity, $forAttributeId = 0)
    {
        // Hook triggered by legacy code below: actionUpdateQuantity('id_product', 'id_product_attribute', 'quantity')
        \StockAvailable::setShopQuantity((int)$product->id, $forAttributeId, $shop_quantity);
        // \HookCore::exec('actionProductUpdate', array('id_product' => (int)$product->id, 'product' => $product));
    }
