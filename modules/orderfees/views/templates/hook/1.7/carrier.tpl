        ...
        <p class="checkbox">
            {if ($fee["obj"]->is_fee & $module->getConstant('IS_OPTION')) && ($fee["obj"]->display_selectable & $module->getConstant('CONTEXT_ADDRESS'))}
                <span class="custom-checkbox">
                    <input type="checkbox" id="fees_carrier_{$fee.id_cart_rule|intval}" name="fees[]" value="{$fee.id_cart_rule|intval}" {if $fee["is_checked"]}checked="checked"{/if} />
                    <span><i class="material-icons checkbox-checked">&#xE5CA;</i></span>
                    <label for="fees_carrier_{$fee.id_cart_rule|intval}">
