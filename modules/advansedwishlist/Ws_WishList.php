...
class Ws_WishList extends ObjectModel
{
    ...
    {
        Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'ws_wishlist_email` WHERE `id_wishlist` = '.(int)($this->id));
        Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'ws_wishlist_product` WHERE `id_wishlist` = '.(int)($this->id));
        if ($this->default) {
            $result = Db::getInstance()->executeS('SELECT * FROM `'._DB_PREFIX_.'ws_wishlist` WHERE `id_customer` = '.(int)$this->id_customer.' AND `id_wishlist` != '.(int)$this->id.' LIMIT 1');
            foreach ($result as $res) {
                Db::getInstance()->update('ws_wishlist', array('default' => '1'), 'id_wishlist = '.(int)$res['id_wishlist']);
            }
        }
    ...
    public static function incCounter($id_wishlist)
    {
        ...
        return Db::getInstance()->execute(
            'UPDATE `'._DB_PREFIX_.'ws_wishlist` SET
    ...
    * Get Customers having a wishlist
        ...
        if (!Cache::isStored($cache_id)) {
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
                'SELECT c.`id_customer`, c.`firstname`, c.`lastname`
                 FROM `'._DB_PREFIX_.'ws_wishlist` w
    ...
    public static function getByIdCustomer($id_customer)
    {
        ...
        $cache_id = 'Ws_WhishList::getByIdCustomer_'.(int)$id_customer.'-'.(int)Shop::getContextShopID().'-'.(int)Shop::getContextShopGroupID();
        if (!Cache::isStored($cache_id)) {
    ...
    public static function refreshWishList($id_wishlist)
    {
        ...
        FROM `'._DB_PREFIX_.'ws_wishlist_product_cart` wpc
        JOIN `'._DB_PREFIX_.'ws_wishlist_product` wp ON (wp.id_wishlist_product = wpc.id_wishlist_product)
        ...
        $freshwish = Db::getInstance()->executeS(
            ...
            FROM `'._DB_PREFIX_.'ws_wishlist_product_cart` wpc
            JOIN `'._DB_PREFIX_.'ws_wishlist_product` wp ON (wpc.id_wishlist_product = wp.id_wishlist_product)
            JOIN `'._DB_PREFIX_.'cart` c ON (c.id_cart = wpc.id_cart)
        ...
        if (isset($res) and $res != false) {
            foreach ($res as $refresh) {
                if ($refresh['wish_quantity'] > $refresh['cart_quantity']) {
                    Db::getInstance()->execute(
                        'UPDATE `'._DB_PREFIX_.'ws_wishlist_product`
                    ...
                    Db::getInstance()->execute(
                        'UPDATE `'._DB_PREFIX_.'ws_wishlist_product_cart`
        ...
        if (isset($freshwish) and $freshwish != false) {
            foreach ($freshwish as $prodcustomer) {
                Db::getInstance()->execute(
                    'UPDATE `'._DB_PREFIX_.'ws_wishlist_product` SET `quantity`=`quantity` +
                    (
    ...
    public static function getProductByIdCustomer($id_wishlist, $id_customer, $id_lang, $id_product = null, $quantity = false)
    {
        ...
        $products = Db::getInstance()->executeS(
            ...
            FROM `'._DB_PREFIX_.'ws_wishlist_product` wp
            ...
            LEFT JOIN `'._DB_PREFIX_.'ws_wishlist` w ON w.`id_wishlist` = wp.`id_wishlist`
    ...
    public static function getInfosByIdCustomer($id_customer)
    {
        ...
        return (Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
          ...
          FROM `'._DB_PREFIX_.'ws_wishlist_product` wp
        INNER JOIN `'._DB_PREFIX_.'ws_wishlist` w ON (w.`id_wishlist` = wp.`id_wishlist`)
        WHERE w.`id_customer` = '.(int)($id_customer).'
        '.$shop_restriction.'
    ...
    public static function issetProduct($id_wishlist, $id_product)
    {
    
        return (Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
            'SELECT SUM(wp.`quantity`) AS nbProducts
          FROM `'._DB_PREFIX_.'ws_wishlist_product` wp
        WHERE wp.`id_wishlist` = '.(int)($id_wishlist).'
    ...
    public static function addProduct($id_wishlist, $id_customer, $id_product, $id_product_attribute, $quantity)
    {
        ...
        $result = Db::getInstance()->getRow(
            'SELECT wp.`quantity`
          FROM `'._DB_PREFIX_.'ws_wishlist_product` wp
        JOIN `'._DB_PREFIX_.'ws_wishlist` w ON (w.`id_wishlist` = wp.`id_wishlist`)
        WHERE wp.`id_wishlist` = '.(int)($id_wishlist).'
        ...
        if (empty($result) === false and sizeof($result)) {
            ...
            return (Db::getInstance()->execute(
                'INSERT INTO `'._DB_PREFIX_.'ws_wishlist_product` (`id_wishlist`, `id_product`, `id_product_attribute`, `quantity`, `priority`) VALUES(
            '.(int)($id_wishlist).',
            '.(int)($id_product).',
    ...
    public static function removeProduct($id_wishlist, $id_customer, $id_product, $id_product_attribute)
    {
        ...
        FROM `'._DB_PREFIX_.'ws_wishlist` w
        LEFT JOIN `'._DB_PREFIX_.'ws_wishlist_product` wp ON (wp.`id_wishlist` = w.`id_wishlist`)
        WHERE `id_customer` = '.(int)($id_customer).'
        ...
        Db::getInstance()->execute(
            'DELETE FROM `'._DB_PREFIX_.'ws_wishlist_product_cart`
        WHERE `id_wishlist_product` = '.(int)($result['id_wishlist_product'])
        );
        return Db::getInstance()->execute(
            'DELETE FROM `'._DB_PREFIX_.'ws_wishlist_product`
        WHERE `id_wishlist` = '.(int)($id_wishlist).'
        AND `id_product` = '.(int)($id_product).'
        AND `id_product_attribute` = '.(int)($id_product_attribute)
        );
    ...
    public static function addBoughtProduct($id_wishlist, $id_product, $id_product_attribute, $id_cart, $quantity)
    {
        ...
        if (!sizeof($result) or
            ...
            Db::getInstance()->executeS(
                'SELECT *
            FROM `'._DB_PREFIX_.'ws_wishlist_product_cart`
        ...
        if (Db::getInstance()->NumRows() > 0) {
            ...
            $result2 = Db::getInstance()->execute(
                'INSERT INTO `'._DB_PREFIX_.'ws_wishlist_product_cart`
    ...
    public static function addEmail($id_wishlist, $email)
    {
        ...
        return (Db::getInstance()->execute(
            'INSERT INTO `'._DB_PREFIX_.'ws_wishlist_email` (`id_wishlist`, `email`, `date_add`) VALUES(
        '.(int)($id_wishlist).',
        \''.pSQL($email).'\',
        \''.pSQL(date('Y-m-d H:i:s')).'\')'
        )
    ...
    public static function getEmail($id_wishlist, $id_customer)
    {
        ...
        return (Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
          ...
          FROM `'._DB_PREFIX_.'ws_wishlist_email` we
        INNER JOIN `'._DB_PREFIX_.'ws_wishlist` w ON w.`id_wishlist` = we.`id_wishlist`
    ...
    */
    ...
    {
        return (Bool)Db::getInstance()->getValue('SELECT * FROM `'._DB_PREFIX_.'ws_wishlist` WHERE `id_customer` = '.$id_customer.' AND `default` = 1');
    }

    public static function getDefault($id_customer)
    {
        return Db::getInstance()->executeS('SELECT * FROM `'._DB_PREFIX_.'ws_wishlist` WHERE `id_customer` = '.$id_customer.' AND `default` = 1');
    }
    ...
    */
    public function setDefault()
    {
        if ($default = $this->getDefault($this->id_customer)) {
            Db::getInstance()->update('ws_wishlist', array('default' => '0'), 'id_wishlist = '.$default[0]['id_wishlist']);
        }

        return Db::getInstance()->update('ws_wishlist', array('default' => '1'), 'id_wishlist = '.$this->id);
    }
}
