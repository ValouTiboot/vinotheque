...
class Chronopost extends CarrierModule
{
    ...
    public static function createCarrier($code)
    {
        ...
        if ($carrier->add()) {
            ...
            //copy logo
            if (!@copy(dirname(__FILE__).'/views/img/carriers/'.Tools::strtolower($code).'.jpg', _PS_SHIP_IMG_DIR_.'/'.(int)$carrier->id.'.jpg')) {
                if (!@copy(dirname(__FILE__).'/views/img/carriers/chronopost.jpg', _PS_SHIP_IMG_DIR_.'/'.(int)$carrier->id.'.jpg')) {
                    return false;
                }
            }
        } else {
    ...
    public static function getSkybillDetails($order, $is_return = false)
    {
        ...
        switch ($carrier->id_reference) {
            ...
            case Configuration::get('CHRONOPOST_CHRONORDV_ID'):
                ...
                $result['productCode'] = $res[0]['product_code'];
                $result['service'] = $res[0]['service_code'];
                $result['timeSlotStartDate'] = $res[0]['delivery_date'];
                $result['timeSlotEndDate'] = $timeSlot->format(DateTime::RFC3339);
    ...
    public static function amountToInsure($id_order)
    {
        ...
        if ($order->total_products < (float)Configuration::get('CHRONOPOST_ADVALOREM_MINVALUE')) {
            return 0;
        }
    ...
    public function hookNewOrder($params)
    {
        ...
        if (Chronopost::isRDV($params['order']->id_carrier)) {
            ...
            $res = $ws->confirmDeliverySlot($query);
...
*/
            ...
            DB::getInstance()->execute('UPDATE '._DB_PREFIX_.'chrono_cart_creneau SET 
                product_code="'.pSQL((string)$res->return->productService->productCode).'",
                service_code="'.pSQL((string)$res->return->productService->serviceCode).'"
                WHERE id_cart='.(int)$params['cart']->id);
        }
    }
    ...
    public function hookBackOfficeHeader($params)
    {
        ...
        return '<script src="https://maps.googleapis.com/maps/api/js?key='. Configuration::get('CHRONOPOST_MAP_APIKEY') .'"></script><script>
    ...
    public static function getRDVCost($id_cart, $tariffLevel = null, $shipping_cost = 0)
    {
        ...
        if ($shipping_cost == 0) {
            ...
            $shipping_cost = $cart->getOrderShippingCost($rdv_id) - Configuration::get('CHRONOPOST_RDV_PRICE'.$sub_tariff);
        }
    ...
    public static function calculateProducts($cart)
    {
        ...
        $res = array('chrono10' => false, 'chronoclassic' => false, 'chrono18' => false, 'relaiseurope' => false,
            'relaisdom' => true, 'rdv' => false, 'sameday' => false);
        ...
        if (empty($cache) || $cache[0]['last_updated'] + 24 * 3600 < time()) {
            ...
            if (empty($cache)) {
                $sql = 'INSERT INTO `'._DB_PREFIX_.'chrono_calculateproducts_cache2`
                    (`postcode`,`country`, `chrono10`,`chrono18`, `chronoclassic`, `relaiseurope`, `relaisdom`, `rdv`, `sameday`, `last_updated`) VALUES
                    ("'.pSQL($a->postcode).'",
                    "'.pSQL($c->iso_code).'",
        ...
        if ($res->return->amountTTC != 0) {
            if (empty($cache)) {
                ...
                DB::getInstance()->query('UPDATE '._DB_PREFIX_.'chrono_quickcost_cache 
                    ...
                    AND weight="'.(float)$cart->getTotalWeight().'"
                ');
            }
    ...
    public function displayForm()
    {
        ...
        $this->context->smarty->assign(
            array(
                ...
                'day_rdv_close_start' => $this->_dayField('day_close_st', 0, 'rdv'),
                'hour_rdv_close_start' => $this->_hourField('hour_close_st', 0, 'rdv'),
                'minute_rdv_close_start' => $this->_minuteField('minute_close_st', 0, 'rdv'),
                ...
                'return_form' => $this->_generateChronoForm('return'),
                'return_default' => Configuration::get('CHRONOPOST_RETURN_DEFAULT')
            )
        );
