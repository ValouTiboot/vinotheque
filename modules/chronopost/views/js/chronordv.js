...
function toggleRDVpane(cust_address, codePostal, city, e)
{
    if($("input.delivery_option_radio:checked").val()==rdv_carrierID+"," || $("input[name=id_carrier]:checked").val()==rdv_carrierIntID) 
    {
        ...
        $('input[name="chronoRDVSlot"]').change(function() {
            ...
            associateCreneau(rank, deliveryDate, slotCode, tariffLevel, transactionID, fee);
        });
...
function associateCreneau(rank, deliveryDate, slotCode, tariffLevel, transactionID, fee)
{
    $.ajax({
        url: path+'/async/storeCreneau.php?rank='+rank+'&deliveryDate='+encodeURIComponent(deliveryDate)+'&slotCode='+encodeURIComponent(slotCode)+'&tariffLevel='+tariffLevel+'&transactionID='+encodeURIComponent(transactionID)+'&fee='+fee+'&cartID='+cartID
    });    
}
