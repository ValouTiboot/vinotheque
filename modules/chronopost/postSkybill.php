...
function createLT($orderid, $totalnb = 1, $isReturn = false)
{
    ...
    if ($isReturn) {
        if (Tools::getValue('return_address') == chronopost::$RETURN_ADDRESS_RETURN) {
            $addressKey = 'RETURN';
        } else if (Tools::getValue('return_address') == chronopost::$RETURN_ADDRESS_INVOICE) {
            $addressKey = 'CUSTOMER';
        } else if (Tools::getValue('return_address') == chronopost::$RETURN_ADDRESS_SHIPPING) {
            $addressKey = 'SHIPPER';
        }
    ...
    $r = $service->shippingV3($params)->return;
