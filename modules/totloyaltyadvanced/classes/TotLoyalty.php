...
class TotLoyalty extends ObjectModel
{
    ...
    public static function getByOrderId($id_order)
    {
        ...
        $result = Db::getInstance()->getRow('SELECT f.id_loyalty FROM `'._DB_PREFIX_.'totloyalty` f WHERE f.id_order = '.(int)($id_order));
    ...
    public static function getPointsByCustomer($id_customer)
    {
        ...
        AND f.id_loyalty_state IN ('.(int)(TotLoyaltyState::getValidationId()).', '.(int)(TotLoyaltyState::getNoneAwardId()).')
        '.$sql_period)
            +
            Db::getInstance()->getValue('
        SELECT SUM(f.points) points
        FROM `'._DB_PREFIX_.'totloyalty` f
        WHERE f.id_customer = '.(int)($id_customer).'
        AND f.id_loyalty_state = '.(int)TotLoyaltyState::getCancelId().'
        AND points < 0
        '.$sql_period);
    }
    ...
    public static function getAllByIdCustomer($id_customer, $id_lang, $onlyValidate = false, $pagination = false, $nb = 10, $page = 1)
    {
        ...
        LEFT JOIN `'._DB_PREFIX_.'totloyalty_state_lang` fsl ON (f.id_loyalty_state = fsl.id_loyalty_state AND fsl.id_lang = '.(int)($id_lang).')
        WHERE f.id_customer = '.(int)($id_customer).$sql_period;
        if ($onlyValidate === true) {
            $query .= ' AND f.id_loyalty_state = '.(int)TotLoyaltyState::getValidationId();
        }
    ...
    public static function registerDiscount($cartRule)
    {
        ...
        foreach ($items as $item) {
            $lm = new TotLoyalty((int)$item['id_loyalty']);

            /* Check for negative points for this order */
            $negativePoints = (int)Db::getInstance()->getValue('SELECT SUM(points) points FROM '._DB_PREFIX_.'totloyalty WHERE id_order = '.(int)$item['id'].' AND id_loyalty_state = '.(int)TotLoyaltyState::getCancelId().' AND points < 0');
            ...
            $lm->id_loyalty_state = (int)TotLoyaltyState::getConvertId();
            $lm->save();
            $associated = true;
        }
        return $associated;
    ...
    public static function getOrdersByIdDiscount($id_cart_rule)
    {
        ...
        FROM `'._DB_PREFIX_.'totloyalty` f
        WHERE f.id_cart_rule = '.(int)$id_cart_rule.' AND f.id_loyalty_state = '.(int)TotLoyaltyState::getConvertId());
    ...
    public static function getVoucher($id_cart_rule)
    {
        $query = ' SELECT * FROM `'._DB_PREFIX_.'cart_cart_rule` WHERE `id_cart_rule` = '.(int)($id_cart_rule);
        $voucherdel = Db::getInstance()->executeS($query);
        if (empty($voucherdel)) {
            $items = Db::getInstance()->executeS('
            SELECT f.id_order AS id_order, f.points AS points, f.date_upd AS date
            FROM `'._DB_PREFIX_.'totloyalty` f
