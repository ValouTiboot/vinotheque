...
class CategoriesClass extends ObjectModel
{
    ...
    public static function getListeNoLang($only_actif = 0, $parent = 0)
    {
        ...
        if (count($liste) > 0) {
            foreach ($liste as $key => $value) {
                if (file_exists($module->module_path.'/views/img/'.Configuration::get($module->name.'_theme').'/up-img/c/'.$value[self::$identifier_static].'.jpg')) {
    ...
    public static function getListeNoArbo($only_actif = 0, $id_lang = 0)
    {
        ...
        if (count($liste) > 0) {
            foreach ($liste as $key => $value) {
                if (file_exists($module->module_path.'/views/img/'.Configuration::get($module->name.'_theme').'/up-img/c/'.(int)$value[self::$identifier_static].'.jpg')) {
                    $liste[$key]['image_presente'] = 1;
                }
            }
        }
    ...
    public function displaySelectArboCategories(
        ...
        foreach ($liste as $value) {
            ...
            $html_out .= '<option '.($branche_disabled ? 'disabled' : 'value="'.$value[self::$identifier_static].'"').' '.((int)$value[self::$identifier_static] == (int)$parent || (int)$value[self::$identifier_static] == (int)$selected ? 'selected' : '').'>';
            ...
            if (count($value['children']) > 0) {
                $html_out .= $this->displaySelectArboCategories($value['children'], $parent, $decalage + 1, $label, $name_select, $on_change, $selected, $branche_disabled, $value['branch']);
            }
        }
