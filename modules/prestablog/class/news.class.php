    ...
    public static function getListe(
        ...
        if (count($liste) > 0) {
            foreach ($liste as $key => $value) {
                ...
                if ((Tools::strlen(trim($value['paragraph'])) == 0) && (Tools::strlen(trim(strip_tags($value['content']))) >= 1)) {
                ...
                if (file_exists($module->module_path.'/views/img/'.Configuration::get($module->name.'_theme').'/up-img/'.$value[self::$identifier_static].'.jpg')) {
                    $liste[$key]['image_presente'] = 1;
                }
                if (Tools::strlen(trim($value['content'])) >= 1) {
                    $liste[$key]['link_for_unique'] = 1;
                }
                $products = self::getProductLinkListe($value['id_prestablog_news'], true);
                if (count($products) > 0)
                    $liste[$key]['product'] = ProductNews::getProducts($products);
            }
        }
    ...
    public function registerTablesBdd()
    {
        ...
        if (count($langues) > 0) {
            ...
            $content = array (
                ...
                quam luctus eu.</h3><p><img src="/modules/prestablog/views/img/demo/photo-demo.jpg" alt="photo de mode"
                ...
                Duis neque dui, vulputate in gravida a, luctus convallis sem.</p>',
            );
