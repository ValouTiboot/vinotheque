...
class LookBookClass extends ObjectModel
{
    ...
    {
        $table = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('SHOW TABLES LIKE \''.bqSQL(_DB_PREFIX_.self::$table_static).'%\'');
    ...
    public static function getListe($id_lang = null, $only_actif = 0)
    {
        ...
        JOIN `'.bqSQL(_DB_PREFIX_.self::$table_static).'_lang` lbl ON (lb.`'.bqSQL(self::$identifier_static).'` = lbl.`'.bqSQL(self::$identifier_static).'`)
        ...
        if (count($liste) > 0) {
            foreach ($liste as $key => $value) {
                ...
                if (Tools::strlen(trim($liste[$key]['description_crop'])) > (int)Configuration::get('prestablog_lb_intro_length')) {
                    $liste[$key]['description_crop'] = PrestaBlog::cleanCut($liste[$key]['description_crop'], (int)Configuration::get('prestablog_lb_intro_length'), ' [...]');
                }

                if (file_exists($module->module_path.'/views/img/'.Configuration::get($module->name.'_theme').'/up-img/lookbook/'.$value[self::$identifier_static].'.jpg')) {
    ...
    public static function getLookBookProducts($id_prestablog_lookbook)
    {
        ...
        LEFT JOIN `'.bqSQL(_DB_PREFIX_.self::$table_static).'` l ON (l.`id_prestablog_lookbook` = lp.`id_prestablog_lookbook`)
    ...
    {
        Db::getInstance(_PS_USE_SQL_SLAVE_)->Execute('
        DELETE FROM `'.bqSQL(_DB_PREFIX_.self::$table_static).'_group` WHERE `id_prestablog_lookbook`='.(int)$lookbook);
    }
