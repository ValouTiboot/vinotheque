                ...
                if (Tools::getValue('req')) {
                    ...
                    if (count($list_product_linked) > 0) {
                        foreach ($list_product_linked as $product_link) {
                            ...
                            $image_thumb_path = ImageManager::thumbnail(_PS_IMG_DIR_.'p/'.$image_product->getExistingImgPath().'.jpg', 'product_mini_'.$product_search->id.'.jpg', 45, 'jpg');
                ...
                if (Tools::getValue('req')) {
                    ...
                    if (count($list_article_linked) > 0) {
                        foreach ($list_article_linked as $article_link) {
                            ...
                            if (file_exists(dirname(__FILE__).'/../../views/img/'.Configuration::get('prestablog_theme').'/up-img/adminth_'.$article_search->id.'.jpg')) {
                                $thumbnail = '<img class="imgm img-thumbnail" src="../modules/prestablog/views/img/'
                                                .Configuration::get('prestablog_theme')
            ...
            case 'searchProducts':
                if (Tools::getValue('req') != '') {
                    if (Tools::strlen(Tools::getValue('req')) >= (int)Configuration::get('prestablog_nb_car_min_linkprod')) {
                        ...
                        // 'description', 'description_short', 'link_rewrite', 'meta_title', 'meta_description', 'meta_keywords'
                            ...
                            echo '
                                <script type="text/javascript">
                                    ...
                                    $("#articleLinkResult img.linked").click(function() {
                                        var idN = $(this).attr("rel");
                                        $("#currentArticleLink").append(
                                            \'<input type="text" name="articlesLink[]" value="\'+idN+\'" class="linked_\'+idN+\'" />\'
                                        );
                    ...
                    if (Tools::strlen(Tools::getValue('req')) >= (int)Configuration::get('prestablog_nb_car_min_linkprod')) {
                        ...
                        if (count($result_search) > 0) {
                            foreach ($result_search as $value) {
                                ...
                                echo '    <tr class="Outlisted noOutlisted_'.$product_search->id.'">
                                                <td class="'.($product_search->active ? '' : 'noactif ').'center">
                                                    <img src="../modules/prestablog/views/img/add.gif" rel="'
                                                        .$product_search->id.'" class="addProductLookbook linked" />
                                                </td>
                                                <td class="'.($product_search->active ? '' : 'noactif ').'center">'.$product_search->id.'</td>
                                                <td class="'.($product_search->active ? '' : 'noactif ').'center" style="width:50px;">'.$image_thumb_path.'</td>
                                                <td class="'.($product_search->active ? '' : 'noactif ').'">'.$product_search->name.'</td>
                            ...
                            echo '
                                <script type="text/javascript">
                                    ...
                                    $(".addProductLookbook").click(function() {
                                        ...
                                        .Tools::getValue('urlReturn').'&addLookbookProduct&idLB='
                                        .(int)Tools::getValue('idLB').'&id_product="+$(this).attr("rel")+"&lookbook_shape="+$("#lookbook_shape").val()+"&lookbook_shape_ed="+$("#lookbook_shape_ed").val();
                                    });
                                </script>'."\n";
                        } else {
                            echo '
