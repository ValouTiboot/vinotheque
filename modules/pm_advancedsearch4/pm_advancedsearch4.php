...
class PM_AdvancedSearch4 extends AdvancedSearchWidgetProxy
{
    ...
    public function getFromSmartyCache($cacheId)
    {
        ...
        if (!empty($config['moduleCache'])) {
            $cacheId = $this->getCacheId($cacheId);
            if ($this->isCached($this->templatePrefix . 'cache.tpl', $cacheId)) {
                if (version_compare(_PS_VERSION_, '1.7.0.0', '>=')) {
                    $cacheData = unserialize(self::getDataUnserialized(trim(strip_tags($this->display(__FILE__, $this->templatePrefix . 'cache.tpl', $cacheId)))));
                } else {
                    $cacheData = unserialize(self::getDataUnserialized($this->display(__FILE__, $this->templatePrefix . 'cache.tpl', $cacheId)));
                }
                if ($cacheData !== false) {
                    return $cacheData;
                }
    ...
    public function setProductFilterContext()
    {
        ...
        } elseif (As4SearchEngine::$productFilterListSource == 'search' || As4SearchEngine::$productFilterListSource == 'jolisearch') {
            if (version_compare(_PS_VERSION_, '1.7.0.0', '>=')) {
    ...
    public function includeAssets()
    {
        ...
        $this->context->smarty->assign(array(
            ...
            'as4_productFilterListData' => (isset(As4SearchEngine::$productFilterListSource) && (As4SearchEngine::$productFilterListSource == 'search' || As4SearchEngine::$productFilterListSource == 'jolisearch') && !empty(As4SearchEngine::$productFilterListData) ? self::getDataSerialized(As4SearchEngine::$productFilterListData) : ''),
            'as_obj' => $this,
            'as_path' => $this->_path,
