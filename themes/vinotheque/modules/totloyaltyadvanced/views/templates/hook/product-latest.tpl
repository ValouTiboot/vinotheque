...
<script type="text/javascript">
	...
	function updateLoyaltyPoints(price)
	{
		...
		if (!none_award && productPriceWithoutReduction != productPrice) {
			...
			var content = "{l s='En achetant ce produit vous cumulez' mod='totloyaltyadvanced'} <b><span id=\"loyalty_points\">"+points+'</span> ';
			if (points > 1) {
				content += "{l s='points' mod='totloyaltyadvanced'}</b> {l s='fidélité' mod='totloyaltyadvanced'} ";
			} else {
				content += "{l s='point' mod='totloyaltyadvanced'}</b> {l s='fidélité' mod='totloyaltyadvanced'} ";
			}
