...
// <![CDATA[
	ThickboxI18nClose = "{l s='Close' mod='referralbyphone'}";
	ThickboxI18nOrEscKey = "{l s='or Esc key' mod='referralbyphone'}";
	tb_pathToImage = "{$img_ps_dir|escape:'htmlall':'UTF-8'}loadingAnimation.gif";
	//]]>
</script>
...
<h2>{l s='Referral program' mod='referralbyphone'}</h2>
...
{if $error}
	<p class="error">
		...
		{elseif $error == 'email exists'}
			{l s='Someone with this e-mail address has already been sponsored!' mod='referralbyphone'}: {foreach from=$mails_exists item=mail}{$mail|escape:'htmlall':'UTF-8'} {/foreach}
...
<ul class="idTabs clearfix">
	<li><a href="#idTab1" title="{l s='Sponsor my friends' mod='referralbyphone'}" {if $activeTab eq 'sponsor'}class="selected"{/if} rel="nofollow">{l s='Sponsor my friends' mod='referralbyphone'}</a></li>
	<li><a href="#idTab2" title="{l s='List of pending friends' mod='referralbyphone'}" {if $activeTab eq 'pending'}class="selected"{/if} rel="nofollow">{l s='Pending friends' mod='referralbyphone'}</a></li>
	<li><a href="#idTab3" title="{l s='List of friends I sponsored' mod='referralbyphone'}" {if $activeTab eq 'subscribed'}class="selected"{/if} rel="nofollow">{l s='Friends I sponsored' mod='referralbyphone'}</a></li>
</ul>
<div class="sheets">
	...
	<div id="idTab1">
		...
		{if $canSendInvitations}
			<p>
                {l s='It\'s quick and it\'s easy.' mod='referralbyphone'}<br/>
                {l s='You can invite your friends in different ways:' mod='referralbyphone'}
                <ul>
                <li class="reff_way"><i class="icon-check-circle-o"></i>{l s='Give them your mail to enter in the registration form.' mod='referralbyphone'}</li>
                <li class="reff_way"><i class="icon-check-circle-o"></i>{l s='Just fill in the first name, last name, and e-mail address(es) of your friend(s) in the fields below.' mod='referralbyphone'}</li>
                </ul>			</p>
			<form method="post" action="{$link->getModuleLink('referralbyphone', 'program', [], true)|escape:'htmlall':'UTF-8'}" class="std">
				...
				<tbody>
					...
					<tr class="{if $smarty.section.friends.index % 2}item{else}alternate_item{/if}">
						...
						<td><input type="text" class="text" name="friendsLastName[{$smarty.section.friends.index|escape:'htmlall':'UTF-8'}]" size="14" value="{if isset($smarty.post.friendsLastName[$smarty.section.friends.index])}{$smarty.post.friendsLastName[$smarty.section.friends.index]|escape:'html':'UTF-8'}{/if}" /></td>
						<td><input type="text" class="text" name="friendsFirstName[{$smarty.section.friends.index|escape:'htmlall':'UTF-8'}]" size="14" value="{if isset($smarty.post.friendsFirstName[$smarty.section.friends.index])}{$smarty.post.friendsFirstName[$smarty.section.friends.index]|escape:'html':'UTF-8'}{/if}" /></td>
						<td><input type="text" class="text" name="friendsEmail[{$smarty.section.friends.index|escape:'htmlall':'UTF-8'}]" size="20" value="{if isset($smarty.post.friendsEmail[$smarty.section.friends.index])}{$smarty.post.friendsEmail[$smarty.section.friends.index]|escape:'html':'UTF-8'}{/if}" /></td>
					</tr>
					{/section}
				</tbody>
				</table>
				...
				<p class="checkbox">
					...
					<a href="{$link->getModuleLink('referralbyphone', 'rules', ['height' => '500', 'width' => '400'], true)|escape:'htmlall':'UTF-8'}" class="thickbox" title="{l s='Conditions of the referral program' mod='referralbyphone'}" rel="nofollow">{l s='Read conditions.' mod='referralbyphone'}</a>
				</p>
				<p class="see_email">
				...
				<p class="submit">
					<input type="submit" id="submitSponsorFriends" name="submitSponsorFriends" class="button_large" value="{l s='Validate' mod='referralbyphone'}" />
				</p>
			</form>
		{else}
			<p class="warning">
	...
	{if $pendingFriends AND $pendingFriends|@count > 0}
		...
		<form method="post" action="{$link->getModuleLink('referralbyphone', 'program', [], true)|escape:'htmlall':'UTF-8'}" class="std">
			...
			<p class="submit">
				<input type="submit" value="{l s='Remind my friend(s)' mod='referralbyphone'}" name="revive" id="revive" class="button_large" />
			</p>
		</form>
		{else}
			<p class="warning">{l s='You have not sponsored any friends.' mod='referralbyphone'}</p>
		{/if}
	</div>
...
<ul class="footer_links">
	<li>
		<a href="{$link->getPageLink('my-account', true)|escape:'htmlall':'UTF-8'}" title="{l s='Back to Your Account' mod='referralbyphone'}" rel="nofollow">
		<img src="{$img_dir|escape:'htmlall':'UTF-8'}icon/my-account.gif" alt="" class="icon" /> {l s='Back to Your Account' mod='referralbyphone'}</a>
	</li>
	<li class="f_right"><a href="{$base_dir|escape:'htmlall':'UTF-8'}" title="{l s='Home' mod='referralbyphone'}"><img src="{$img_dir|escape:'htmlall':'UTF-8'}icon/home.gif" alt="" class="icon" /> {l s='Home' mod='referralbyphone'}</a></li>
</ul>
